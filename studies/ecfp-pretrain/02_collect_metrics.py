#!/usr/bin/env python
"""Step 2: gather every prediction and reduce it to per-fold statistics.

Reads every tidy prediction file under predictions/<ds>/ -- the four baseline arms
imported in step 0 and the PT-GIN arm from step 1 -- concatenates them into
results/<ds>/predictions_all.parquet, and computes R^2, Spearman rho and MAE for
each (endpoint, method, repeat, fold) on the fixed test set.

That gives results/<ds>/fold_metrics.csv: 9 x 5 x 25 = 1,125 rows on ExpansionRx
and 6 x 5 x 25 = 750 on Biogen when both sweeps are complete.

Runs against a partial sweep as well; it reports what is missing rather than
failing, so progress can be checked while step 1 is still going.

    python 02_collect_metrics.py
    ADME_DATASET=biogen python 02_collect_metrics.py
    python 02_collect_metrics.py --no-parquet   # metrics only, skip the big file
"""
...
def load_predictions() -> pd.DataFrame:
    ...
    if not paths:
        raise SystemExit(f"no prediction files under {cfg.PRED_DIR} -- run steps 0 and 1 first")
...
def report_coverage(metrics: pd.DataFrame) -> None:
    ...
    if incomplete:
        ...
        print(f"\ncomplete: {len(metrics)} rows "
              f"({len(cfg.TARGET_COLS)} x {len(cfg.ALL_METHODS)} x {expected})")
...
def summarise(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        ...
        .agg(["mean", "std"])
        .reindex(pd.MultiIndex.from_product([cfg.TARGET_COLS, cfg.ALL_METHODS],
                                            names=["endpoint", "method"]))
        .dropna(how="all")
    )
...
def main() -> None:
    ...
    summary.to_csv(cfg.TABLE_DIR / "summary_raw.csv", index=False)
